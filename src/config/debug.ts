...
const physicsDebugger = (e: KeyboardEvent, scene: Scene) => {
  if (
    ...
    scene.meshes.forEach(mesh => {
      if (mesh.physicsImpostor) physicsViewer.showImpostor(mesh.physicsImpostor);
    })
  }
}
...
const changeCamera = (scene: Scene) => {
  ...
  const button = Button.CreateImageOnlyButton('switchCamera', '/images/debug/change-camera.png');
  button.width = "100px";
  button.height = "55px";
